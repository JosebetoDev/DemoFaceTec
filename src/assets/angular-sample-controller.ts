...
export const AngularSampleApp = (function() {
  ...
  window.onload = function() {
    ...
    FaceTecSDK.initializeInDevelopmentMode(KeysFace.DeviceKeyIdentifier,KeysFace.PublicFaceScanEncryptionKey,function(initializedSuccessfully: boolean) {
  ...
  function onLivenessCheckPressed() {
    ...
    getSessionToken(function(sessionToken) {
      ...
      console.log("AngularSample: ",AngularSampleApp);
      console.log("Obtiene la sesion del token:",latestProcessor);
    });
  }
  ...
  function getSessionToken(sessionTokenCallback: (sessionToken?: string)=>void) {
    ...
    XHR.open("GET", KeysFace.BaseURL + "/session-token");
    XHR.setRequestHeader("X-Device-Key", KeysFace.DeviceKeyIdentifier);
